...
def write_cube_file(filename, density, x, y, z, nuclei, parser):
    ...
    with open(file, 'w') as f:
        ...
        for atom in nuclei:
            z = atom.get('atomic_number', atom.get('Z'))
            if z is None:
                raise KeyError("Nucleus entry missing 'atomic_number'.")
            coords_bohr = np.array(atom['coords'])
            f.write(f"{int(z):5d} {0.0:12.6f} {coords_bohr[0]:12.6f} {coords_bohr[1]:12.6f} {coords_bohr[2]:12.6f}\n")
